...
const descargarPDF = async (simulacion) => {
  try {
    ...
    doc.text('ID de Simulación:', 14, currentY + 6)
    doc.text('Fecha de Emisión:', 14, currentY + 12)

    doc.setFont('helvetica', 'bold')
    doc.setTextColor(...textColor)
    doc.text(`#SIM-${(simulacion.simulacionId || 0).toString().padStart(8, '0')}`, 50, currentY + 6)
    doc.text(
    ...
    <div v-else class="table-container">
      <table class="financial-table interactive">
        ...
        <tbody>
          ...
          >
            <td class="col-id">#{{ (s.simulacionId || 0).toString().padStart(6, '0') }}</td>
            <td class="col-divisa">
